  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[{label:"Total Gross",value:formatCurrency(payrollSummary.totalGross),icon:<IndianRupee className="w-5 h-5" />,color:"#ff6a2c",trend:"+1.7%",trendUp:true},{label:"Total Net",value:formatCurrency(payrollSummary.totalNet),icon:<Wallet className="w-5 h-5" />,color:"#10b981",trend:"+2.1%",trendUp:true},{label:"Total Deductions",value:formatCurrency(payrollSummary.totalDeductions),icon:<TrendingDown className="w-5 h-5" />,color:"#f59e0b",trend:"+0.5%",trendUp:false},{label:"Employees Processed",value:`${payrollSummary.employeesProcessed}/${payrollSummary.totalEmployees}`,icon:<Users className="w-5 h-5" />,color:"#8b5cf6",trend:"97.4%",trendUp:true}].map((card,i)=>(
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="show"><Card className="rounded-[24px] border-0 shadow-lg h-full"><CardHeader className="pb-2"><div className="flex items-center justify-between"><CardTitle className="text-lg font-semibold flex items-center gap-2"><Briefcase className="w-5 h-5 text-[var(--saptta-accent)]" />Structure Templates</CardTitle><Button size="sm" className="rounded-full text-xs bg-[var(--saptta-accent)] hover:bg-[var(--saptta-accent)]/90 text-white"><Plus className="w-3.5 h-3.5 mr-1" />New Template</Button></div></CardHeader><CardContent><div className="space-y-3">{structureTemplates.map((tpl)=>(<div key={tpl.name} className="flex items-center justify-between p-4 rounded-2xl border border-[var(--saptta-line)] hover:border-[var(--saptta-accent)]/30 transition-colors"><div><h4 className="font-medium text-sm">{tpl.name}</h4><p className="text-xs text-[var(--saptta-mute)]">{tpl.ctc} · {tpl.employees} employees</p></div><div className="flex items-center gap-2"><Badge variant="secondary" className="rounded-full text-xs">{tpl.dept}</Badge><Button variant="ghost" size="sm" className="text-xs text-[var(--saptta-accent)] h-7"><Eye className="w-3 h-3 mr-1" />View</Button></div></div>))}</div><div className="mt-4 pt-4 border-t border-[var(--saptta-line)]"><Button variant="outline" className="w-full rounded-full"><Upload className="w-4 h-4 mr-2" />Bulk Assign Structures</Button></div></CardContent></Card></motion.div>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-6">
      <motion.div variants={fadeUp} initial="hidden" animate="show"><Card className="rounded-[24px] border-0 shadow-lg"><CardContent className="p-6">
        ...
        <AnimatePresence mode="wait">
          ...
          {currentStep===4&&<motion.div key="s4" initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-20}}><h3 className="text-lg font-semibold mb-4">Lock & Generate Payslips</h3><div className="text-center space-y-4 py-4"><div className="w-20 h-20 mx-auto rounded-full bg-emerald-100 flex items-center justify-center"><Lock className="w-10 h-10 text-emerald-600" /></div><p className="text-sm text-[var(--saptta-mute)]">Locking payroll will finalize all calculations and generate payslips for 1,247 employees.</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg mx-auto"><div className="p-4 rounded-2xl bg-[var(--saptta-bg-2)]"><p className="text-xs text-[var(--saptta-mute)]">Bank File (NEFT)</p><Button variant="outline" size="sm" className="mt-2 rounded-full text-xs w-full"><Download className="w-3 h-3 mr-1" />Generate NEFT File</Button></div><div className="p-4 rounded-2xl bg-[var(--saptta-bg-2)]"><p className="text-xs text-[var(--saptta-mute)]">Payslip Emails</p><Button variant="outline" size="sm" className="mt-2 rounded-full text-xs w-full"><Mail className="w-3 h-3 mr-1" />Send Payslips</Button></div></div></div></motion.div>}
        </AnimatePresence>
