...
function PayslipsTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"><div className="flex items-center gap-2 w-full sm:w-auto"><div className="relative flex-1 sm:w-80"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[var(--saptta-mute)]" /><Input placeholder="Search employee..." className="rounded-full pl-9" /></div><Select defaultValue="jun-2026"><SelectTrigger className="w-[130px] rounded-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="jun-2026">Jun 2026</SelectItem><SelectItem value="may-2026">May 2026</SelectItem></SelectContent></Select></div><div className="flex items-center gap-2"><Button variant="outline" onClick={() => toast.success("Downloading all payslips…")} className="rounded-full text-xs"><Download className="w-3.5 h-3.5 mr-1" />Bulk Download</Button><Button variant="outline" onClick={() => toast.success("Sending payslips via email…")} className="rounded-full text-xs"><Mail className="w-3.5 h-3.5 mr-1" />Email Payslips</Button></div></div>
